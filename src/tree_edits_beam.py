...
def sample(complex_sentences, simple_sentences, input_lang, tag_lang, dep_lang, lm_forward, lm_backward, embedding_weights, idf, unigram_prob, start_time):
    count = 0
    sari_scorel = 0
    keepl = 0
    deletel = 0
    ...
    all_par_calls = 0
    beam_calls = 0
    stats = {'ls':0, 'dl':0, 'las':0, 'rl':0, 'par':0} #changed
...
def mcmc(input_sent, reference, input_lang, tag_lang, dep_lang, lm_forward, lm_backward, embedding_weights, idf, unigram_prob, stats):
    ...
    # new_testing
    return scorel, keepl, deletel, addl, bleul, perpf, fkgl_scorel, fre_scorel, all_par_calls, beam_calls
